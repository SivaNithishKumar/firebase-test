...
import { UserPlus, CheckCircle, Clock, Loader2, Users as FriendsIcon, Search, UserRoundPlus } from "lucide-react";

const convertAppUserProfileTimestamp = (profile: any): AppUserProfile => {
    if (!profile) return { uid: '', displayName: 'Anonymous', email: null, photoURL: null, createdAt: Date.now(), friends: [] };
    return {
        ...profile,
        ...
        try {
          ...
          if (userProfileSnap.exists()) {
            setCurrentUserProfile(convertAppUserProfileTimestamp(userProfileSnap.data()));
             ...
             console.warn(`[FriendsPage] Current user profile not found for UID: ${user.uid}. Attempting to create one.`);
             const basicProfileData: Omit<AppUserProfile, 'createdAt'> & {createdAt: any} = {
              ...
              // @ts-ignore
              await setDoc(userProfileRef, basicProfileData);
              setCurrentUserProfile(convertAppUserProfileTimestamp({ ...basicProfileData, createdAt: Date.now() })); // Temp client-side version
          }

          const usersQuery = query(collection(db, "userProfiles"), where("uid", "!=", user.uid));
