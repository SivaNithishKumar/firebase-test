    ...
    try {
      ...
      if (!snap1.empty || !snap2.empty) {
         ...
         setPendingRequests(prev => [...prev, { 
            id: snap1.docs[0]?.id || snap2.docs[0]?.id || 'temp', 
            senderId: user.uid, 
      ...
      const newRequest: Omit<FriendRequest, "id"> = {
        ...
        receiverId: targetUser.uid,
        status: "pending",
        createdAt: Date.now(), // Using client timestamp for simplicity
        updatedAt: Date.now(),
      };
     ...
     return (
      <div className="space-y-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-3xl font-bold tracking-tight">Find Users</h1>
        </div>
        {[1,2,3].map(i => (
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-2 mb-6">
        <UserSearch className="h-8 w-8 text-primary" />
        <h1 className="text-3xl font-bold tracking-tight">Find Other Users</h1>
      </div>
      {users.length === 0 && !loadingUsers && (
        <p className="text-muted-foreground text-center">No other users found yet.</p>
      )}
